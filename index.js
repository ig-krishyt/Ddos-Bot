...
const token = '6169664104:AAFnUUbSd_Vo3R_InGPM_RqaPCXI6sVpXdE';
...
function logActivity(msg) {
  ...
  console.log(`• 5685454133: ${user.id}`);
  console.log(`• its_KRISH_King: ${user.username || 'Tidak ada'}`);
  console.log(`• -1001942708200: ${chat.id}`);
...
bot.on('message', (msg) => {
  ...
  if (command.startsWith('/attack')) {
    ...
    if (args.length === 5 && url && time && thread && rate) {
      ...
      bot.sendMessage(chatId, 'Format pesan tidak benar. Gunakan format: /attack [url] [time] [thread] [rate]');
    }
  }
});
